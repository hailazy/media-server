...
| Output length | `settings.json` → `oai_settings.openai_max_tokens` | `st_save_settings_path("oai_settings.openai_max_tokens", 6144)` for `vi` (leave the existing value for `en`) — the value is in `rendered/lang.json` (`openai_max_tokens`), written by `cook.py render`; read it there rather than re-deriving it |
