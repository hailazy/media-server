...
- **Civitai MCP get_model only shows latest version:** Model 833294 (NoobAI XL) returns chỉ V-Pred-1.0 (1190596) qua `get_model`, không list các EPS versions. Cần explicit version_id qua `get_model_version` để fetch EPS 1.1 (version 1116447). Workaround: Civitai web URL `?modelVersionId=<id>` reveals all versions. 2026-05-07.
