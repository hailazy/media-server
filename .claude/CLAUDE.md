...
- **Civitai MCP get_model formatter bug (fix pending):** `get_model` API call returns full `modelVersions[]` array but `format_model_card` formatter chỉ display `versions[0]` (latest). Root cause: `formatters.py:68` hardcoded `v = versions[0]`. PR submitted: https://github.com/timoncool/civitai-mcp-ultimate/pull/1. Workaround hiện tại: dùng `get_model_version(version_id)` với explicit ID. Installed v0.3.0 từ source (2026-05-07) — chưa include fix vì PR chưa merge. 2026-05-07.
