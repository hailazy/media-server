...
check_basic() {
    ...
    if [[ -f ".env" ]]; then
        ...
        if grep -q "PIA_USER=" .env && grep -q "PIA_PASS=" .env; then
            ...
            echo_warn "PIA credentials missing in .env"
        fi
    else
        echo_error "Environment file missing - copy from .env.example"
    fi
...
check_services() {
    ...
    if podman-compose ps &> /dev/null; then
        local running_services=$(podman-compose ps --services --filter status=running | wc -l)
        local total_services=$(podman-compose ps --services | wc -l)
        ...
        if [[ $running_services -eq $total_services ]]; then
            ...
            echo_debug "Run: podman-compose ps"
        fi
        
        # Check for unhealthy services
        local unhealthy=$(podman-compose ps --filter health=unhealthy --format "{{.Service}}" 2>/dev/null || true)
        if [[ -n "$unhealthy" ]]; then
            echo_error "Unhealthy services: $unhealthy"
        fi
    else
        echo_warn "No services running or podman-compose not available"
        echo_debug "Start with: podman-compose up -d"
    fi
}
...
check_vpn() {
    ...
    if podman-compose ps gluetun | grep -q "Up"; then
        echo_info "Gluetun VPN service is running"
        
        # Test VPN IP
        local vpn_ip=$(podman-compose exec -T gluetun wget -qO- https://ipinfo.io/ip 2>/dev/null || echo "failed")
        ...
        if podman-compose exec -T gluetun ls /gluetun/wireguard/wg0.conf &> /dev/null; then
            echo_info "WireGuard config found"
        else
            echo_warn "WireGuard config missing - regenerate with: podman-compose run --rm pia-wggen"
        fi
    else
        echo_error "Gluetun VPN service not running"
        echo_debug "Check logs with: podman-compose logs gluetun"
    fi
}
...
check_port_forwarding() {
    ...
    if podman-compose ps pia-pf | grep -q "Up"; then
        echo_info "PIA port forwarding service is running"
        
        # Check for forwarded port
        local port_file="/tmp/gluetun/forwarded_port"
        local forwarded_port=$(podman-compose exec -T gluetun cat "$port_file" 2>/dev/null | tr -d '\r\n' || echo "")
        
        if [[ -n "$forwarded_port" ]] && [[ "$forwarded_port" =~ ^[0-9]+$ ]]; then
            echo_info "Forwarded port: $forwarded_port"
            
            # Check qBittorrent port
            local qb_port=$(podman-compose exec -T gluetun wget -qO- http://127.0.0.1:8090/api/v2/app/preferences 2>/dev/null | grep -o '"listen_port":[0-9]*' | cut -d: -f2 || echo "unknown")
            ...
            echo_debug "Check logs with: podman-compose logs pia-pf"
        fi
    else
...
show_quick_commands() {
    ...
    echo "Restart VPN:             podman-compose restart gluetun pia-pf"
    echo "Regenerate VPN config:   podman-compose run --rm pia-wggen"
    echo "View all logs:           podman-compose logs -f"
    echo "View VPN logs:           podman-compose logs -f gluetun"
    echo "View port forward logs:  podman-compose logs -f pia-pf"
