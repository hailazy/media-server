...
check_qbittorrent_connectivity() {
    ...
    local qb_response
    qb_response=$(podman-compose exec -T gluetun wget -qO- --timeout=5 http://127.0.0.1:8080/api/v2/app/version 2>/dev/null || echo "failed")
...
run_connectivity_tests() {
    ...
    
    {
        ...
        if podman-compose ps | grep -q "gluetun"; then
            ...
            echo "- qBittorrent via VPN: $(podman-compose exec -T gluetun wget -qO- --timeout=5 http://127.0.0.1:8080 &>/dev/null && echo 'OK' || echo 'FAILED')"
        fi
...
troubleshoot_download_issues() {
    ...
    
    {
        ...
        echo "- Current listening port: $(podman-compose exec -T gluetun wget -qO- http://127.0.0.1:8080/api/v2/app/preferences 2>/dev/null | grep -o '"listen_port":[0-9]*' | cut -d: -f2 || echo 'UNKNOWN')"
        echo ""
