...
- `openrouter_providers: ["StreamLake", "DeepSeek"]`, `openrouter_allow_fallbacks: true` — **order matters, StreamLake first**
...
| Summary returns RP prose / word-salad / blank | OpenRouter routed to a bad provider (DeepInfra, GMICloud) — `openrouter_quantizations` non-empty makes the provider pin useless | `openrouter_quantizations: []` + pin StreamLake first, DeepSeek second (NOT Alibaba — moderates NSFW output; DeepSeek native silently returns EMPTY on some explicit full-context prompts) in live + every preset; clear the summary box before retry — gotcha 5.44 |
