...
### 5.48 Over-constraint kills prose — compliance ≠ pleasure (2026-08-31)

**Symptom:** the 2026-08-30 narrator-contract stack (sim-tuned to 35/36 — gotcha 5.45) fixed railroading but strangled the prose: AI turns fell from 400–974 words (rich, NPC dialogue, complete scenes) to ~300 words of static object description.

**Root cause:** structure rules were pushed into the PROSE layer instead of the Direction/bible layer. The VOICE FENCE (banning the narrator from writing the protagonist's speech/decisions/interiority), the one-beat-per-message cap ("count sensation sentences, delete all but the last"), DOOR novelty rotation ("never repeat a door from the last three messages"), "when a beat needs her word, STOP mid-scene", empty-player-turn formulas beyond "advance time and consequence", and the impersonation content-fence ("she never writes what the narrator owns") + fixed register all optimized for the same thing the sim gate measured: contract COMPLIANCE. The gate scored compliance, not reader pleasure — **compliance ≠ pleasure**, and a reply can obey every rule and still be dead.

**Fix (v3):** structure stays in the Direction/bible layer — menu not beat-sheet, escalation as cost (never volume, monotonic per chapter), novelty ledger, resistance-fails-sideways, continuity guards (never walk back canon, finished objects stay finished), creature-never-speaks + N-GUARD/H-LIMIT. The prose layer is freed: the narrator writes the protagonist WHOLE (body, half-thoughts, speech) ladder-gated to "as far as the creature has earned" (involuntary early → rationalized mid → body-before-will late); interiority is now REQUIRED content, written as "double exposure" self-deception caught in the act (body tells the truth in one clause, her filing system revises it in the next), never banned; pacing = one manga page per reply (multiple beats at the pace the scene earns, page-turn endings, scenes free to span pages — no numeric beat ceilings: limiting variance limits the pleasure); world stays proactive (bystanders, interruptions, NPC dialogue in quotes); style is sensory-dense with concrete nouns, no stock phrases, varying shape across consecutive replies. `mes_example` (2 exchanges, authored in the target voice at cook time) is now REQUIRED on every cooked card — a few-shot style anchor that keeps prose rich without needing prose-layer rules to enforce it. `oai_settings.impersonation_prompt` becomes an enrichment engine with an EVOLVING register (euphemism → naming → wanting → planning) rather than a fixed voice behind a content fence. Single target model throughout: **DeepSeek v4-pro via the StreamLake provider pin** (gotcha 5.44) — no multi-model/multi-provider hedging.

### 5.49 The player needs her memory, not the plot — player brief + GIVENS ON ARRIVAL (2026-09-05)

**Symptom:** 15 messages into Ch1 under v3 — prose finally good (AI turns 415–740 words, NPC dialogue, double exposure) — Hải still reported the narrator "steering chật vật". He plays blind (doesn't read the lorebook) and so had the protagonist's body but not her *memory*: when the neighbour appeared he didn't know who she was to her, feared "what if she's the husband's mistress" (the bible settled that weeks ago — in a gitignored file he never opens), and invented basics mid-scene (floor of the building, where the husband is, a dog) that the lorebook then contradicted.

**Root cause:** the Direction is a beat MENU by design and carries no *givens*; the bible mixes givens with plot, so reading it spoils; the lorebook is narrator-facing. Nothing in the stack was written for the player. Information asymmetry between the two co-authors, not a prose problem.

**Fix:** (1) a **player brief** per campaign (`_scripts/<slug>/player-brief.md`, template `st-cook/assets/player-brief.tmpl`): fixed givens the narrator will honour (home, partner, the cast *as she knows them*, work, body, the creature's calendar, limits) + the open slots that are his + how to cooperate (narrator owns world and clock; short turn = keep going, long turn = I'm driving; "later" isn't written, so nothing he writes can contradict it). No beats, no destination — that split is the point. (2) the card's **GIVENS ON ARRIVAL** clause inside WORLD ALIVE: the first time a person or place enters the page, the narrator folds a line or two of *her* memory of them into the present moment, in her register — never a dossier, never a flashback scene. Across chats it fires once per character and reads the baked Established State, so it recaps the relationship as it stands now, not day one. (3) **chat canon overrides the bible**: whatever the player invented is baked (Established State + keyed cast/place entries) and the bible is patched to match, never the reverse. Bake mid-chapter when the player has invented more than the lorebook holds — don't wait for the chapter to end.
